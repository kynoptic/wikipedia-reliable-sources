...
This reads `outputs/reliability_ranking.csv` (produced by `core.bridge_reliability`) and `goggle_overlay.txt`, then writes both goggle variants plus two reports: `outputs/goggle_diff.md` (what the base generates versus the committed goggle — additions, conflicts, and the overlay-preserved remainder) and `outputs/goggle_gap_candidates.csv` (heavily-cited but unrated domains, for manual review — never auto-added).

On a conflict, the curated overlay value wins and the disagreement is logged to the diff report. Edit `goggle_overlay.txt` by hand to maintain rules the generator can't produce.
